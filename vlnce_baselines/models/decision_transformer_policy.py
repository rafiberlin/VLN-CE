...
class AbstractDecisionTransformerNet(Net):
    ...
    def handle_instruction_embeddings(self, instruction_embedding, resize_tensor, batch_size, seq_length):
...
class DecisionTransformerNet(AbstractDecisionTransformerNet):
    ...
    def handle_instruction_embeddings(self, instruction_embedding, resize_tensor, batch_size, seq_length):
        ...
        instruction_embedding = self.instruction_activation(self.instruction_encoder(instruction_embedding))
...
class FullDecisionTransformerNet(Net):
    """DecisionTransformer with 3 different States Embeddings. Finally, a distribution over discrete
    actions (FWD, L, R, STOP) is produced.
    """
    ...
    def __init__(
        ...
        self.positional_encoding_for_instruction = PositionalEncoding(model_config.DECISION_TRANSFORMER.hidden_dim)
    ...
    def forward(self, observations, rnn_states, prev_actions, masks):
        ...
        output = self.full_transformer(src=instruction_state_embeddings, tgt= stacked_inputs2, src_key_padding_mask=text_mask, tgt_mask=causal_mask)
...
class FullDecisionTransformerNet2(Net):
    """DecisionTransformer with 3 different States Embeddings. Finally, a distribution over discrete
    actions (FWD, L, R, STOP) is produced.
    """
    ...
    def __init__(
        ...
        self.full_transformer = nn.Transformer(d_model=model_config.DECISION_TRANSFORMER.hidden_dim
                                               ...
                                               , activation="gelu"
                                               , batch_first=True)
        self.full_transformer_2 = nn.Transformer(d_model=model_config.DECISION_TRANSFORMER.hidden_dim
                                               , nhead=model_config.DECISION_TRANSFORMER.n_head
                                               , num_encoder_layers=model_config.DECISION_TRANSFORMER.ENCODER.n_layer
                                               , num_decoder_layers=model_config.DECISION_TRANSFORMER.n_layer
                                               , dim_feedforward=model_config.DECISION_TRANSFORMER.hidden_dim * 2
                                               , activation="gelu"
                                               , batch_first=True)
        self.full_transformer_3 = nn.Transformer(d_model=model_config.DECISION_TRANSFORMER.hidden_dim
                                               , nhead=model_config.DECISION_TRANSFORMER.n_head
                                               , num_encoder_layers=model_config.DECISION_TRANSFORMER.ENCODER.n_layer
                                               , num_decoder_layers=model_config.DECISION_TRANSFORMER.n_layer
                                               , dim_feedforward=model_config.DECISION_TRANSFORMER.hidden_dim * 2
    ...
    def forward(self, observations, rnn_states, prev_actions, masks):
        ...
        causal_text_mask = VanillaMultiHeadAttention.create_causal_mask(instruction_state_embeddings.shape[1], instruction_state_embeddings.device)[0][0]
        ...
        output = self.full_transformer(src=instruction_state_embeddings, tgt= stacked_inputs2, src_key_padding_mask=text_mask, tgt_mask=causal_mask)
        output_rgb = self.full_transformer_2(src=rgb_state_embeddings2, tgt= instruction_state_embeddings, src_key_padding_mask=rgb_mask, tgt_mask=causal_text_mask)
        output_depth = self.full_transformer_3(src=depth_state_embeddings2, tgt= instruction_state_embeddings, src_key_padding_mask=depth_mask, tgt_mask=causal_text_mask)
        ...
        dim_for_concat = 1
        cat = lambda t: torch.cat((stacked, t.unsqueeze(dim_for_concat)), dim=dim_for_concat)
        stacked = returns_embeddings2.unsqueeze(dim_for_concat)
        stacked = cat(rgb_state_embeddings2)
        stacked = cat(depth_state_embeddings2)
        stacked = torch.cat((stacked, output), dim=dim_for_concat)
        stacked = cat(output_rgb)
        stacked = cat(output_depth)
        stacked = cat(action_embeddings2).permute(0, 2, 1, 3).reshape(batch_size, self.transformer_step_size* seq_length, -1)
